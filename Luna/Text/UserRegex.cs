...
public struct UserRegex
{
    ...
    public static bool DrawRegexInput(Utf8LabelHandler label, ref UserRegex regex, Utf8HintHandler hint, Action? tooltip = null,
        float width = 0, Rgba32 failureColor = default, RegexOptions options = DefaultOptions)
    {
        bool change;
        var  text       = regex.Text;
        ...
        using (ImStyleBorder.Frame.Push(failureColor, Im.Style.GlobalScale, drawBorder))
        {
            ...
            change = Im.Input.Text(label, ref text, hint) && text != regex.Text;
        }

        if (tooltip != null && Im.Item.Hovered())
            tooltip();
