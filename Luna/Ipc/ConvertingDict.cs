    ...
    where TKey : notnull
{
    ...
    public IEnumerable<TValueTo> Values
        => dict.Values.Select(value => ConvertValue(value));
    ...
    public IEnumerator<KeyValuePair<TKey, TValueTo>> GetEnumerator()
        => dict.Select(kvp => new KeyValuePair<TKey, TValueTo>(kvp.Key, ConvertValue(kvp.Value))).GetEnumerator();
    ...
    where TKeyTo : notnull
{
    ...
    public IEnumerable<TKeyTo> Keys
        => dict.Keys.Select(key => ConvertKey(key));
    ...
    public IEnumerator<KeyValuePair<TKeyTo, TValueTo>> GetEnumerator()
        => dict.Select(kvp => new KeyValuePair<TKeyTo, TValueTo>(ConvertKey(kvp.Key), ConvertValue(kvp.Value))).GetEnumerator();
