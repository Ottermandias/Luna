...
/// <summary> Extensions that deal with implicit power sets. </summary>
public static class PowerSetExtensions
{
    ...
    extension<T1>(IList<T1> set)
    {
        ...
        public bool MoveWithPowerSet<T2>(IList<T2> powerSet, ref int setFrom, ref int setTo)
        {
            ...
            {
                var newIndex = BitFunctions.MoveBit(i, setFrom, setTo);
        ...
        /// <returns> True if anything changed. </returns>
        public bool RemoveWithPowerSet<T2>(IList<T2> powerSet, int setIndex)
        {
        ...
        public bool AddNewWithPowerSet<T2>(IList<T2> powerSet, T1 newItem, Func<T2> generator, int maxSize = 30)
        {
