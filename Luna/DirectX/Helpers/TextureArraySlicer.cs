...
/// <summary> Creates ImGui texture handles over slices of array textures, and manages their lifetime. </summary>
public sealed unsafe class TextureArraySlicer : IUiService, IDisposable
{
    ...
    /// <summary> Create a texture array slicer. </summary>
    public TextureArraySlicer()
        => ImSharpPerFrame.Update += Tick;

    /// <summary> Gets an ImGui texture handle over a slice of the given array texture. </summary>
    /// <param name="texture"> The texture of which to get a slice. </param>
    /// <param name="sliceIndex"> The index of the slice to get. Must be less than <see cref="Texture.ArraySize"/>. </param>
    /// <returns> An ImGui texture handle, to use with all ImGui image functions. </returns>
    ...
    public ImTextureId GetImGuiHandle(Texture* texture, byte sliceIndex)
    {
        ...
        using var srv = new ComPtr<ID3D11ShaderResourceView>((ID3D11ShaderResourceView*)(nint)texture->D3D11ShaderResourceView);
        D3D11_SHADER_RESOURCE_VIEW_DESC description;
        srv.Get()->GetDesc(&description);
        switch (description.ViewDimension)
        {
            ...
            case D3D_SRV_DIMENSION.D3D11_SRV_DIMENSION_TEXTURECUBEARRAY:
                ...
                break;
            default: throw new NotSupportedException($"{nameof(TextureArraySlicer)} does not support dimension {description.ViewDimension}");
        }

        using var device = new ComPtr<ID3D11Device>();
        srv.Get()->GetDevice(device.GetAddressOf());
        using var resource = new ComPtr<ID3D11Resource>();
        srv.Get()->GetResource(resource.GetAddressOf());
        ID3D11ShaderResourceView* slicedSrv = null;
        Marshal.ThrowExceptionForHR(device.Get()->CreateShaderResourceView(resource, &description, &slicedSrv));
    ...
    {
        /// <summary> The shader resource view wrapped by this object. </summary>
